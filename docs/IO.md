...
[メモ]
- 参照等価
  - ある式の値をその式の値で置き換えてもプログラムの結果に影響を与えない
- Effect
  - 2回走らせられないもの
  - このEffectをコントロールするのがCats Effect 3の概念
- Monad
  - プログラミングにおいて順序や依存関係を表現するもの
- Fibers
  - スレッドのように書いた軽量スレッド
  - Cats Effectで書いたプログラミングは全てこのFiberの上で動く
  - スレッドの上に構築、スレッドの上で動くプログラミングの単位
  - メモリの許す限り作成可能
  - キャンセル可能
  - JVMのスレッドはネイティブスレッド
  - JVMのメモリバリアをうまく使ってる
  - 別スレッドにFiberを移す時はExecuterのQueueを使ってる
  - キャンセルの時にレースコンディションが起きないようにアトミックブーリアンを継承していてアトミックブーリアンのメモリバリアを上手いこと利用している
  - IOは便利モナドのようになっている
    - いろんな型がある
      - 全部の機能はいらないので、機能ごとに分けている
    - Syncはブロッキング
    - Asyncはコールバックを受けるやつ
    - MonadCancelはキャンセルできるやつ
  - IOのままだとテストしづらい
    - Free
      - 計算をデータとして表現する
...
- 別スレッドで実行したいメソッドを、スレッドプール専用のキューに次々と入れる（登録する）だけで、後は自動的に登録したメソッドが別スレッドで効率よく実行されていく
- アプリケーションからの要求用に確保するスレッドの最小数と最大数を指定できる
...
- [第 19 章 スレッドプール](https://docs.oracle.com/cd/E19823-01/819-1551/threadpools.html)
